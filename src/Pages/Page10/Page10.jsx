...
function Page10() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${DOMAIN}/api/site-showcase-two/showcase-two`);
        const result = await response.json();
        // User example: direct object: { pageTitle: "...", ... }
        setData(result);
      } catch (error) {
        console.error("Error fetching Page 10 data:", error);
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="w-1/2 flex items-end justify-center">
          {data?.mainImageUrl && (
            <img
                ...
                alt="Right Image"
                className="absolute bottom-40 right-55 w-64 md:w-100 opacity-95 pointer-events-none select-none z-20 object-contain"
            />
          )}
        </div>
      </div>
